...
fed_bank setVariable ["safe",40,true];
fed_bank_1 setVariable ["safe",40,true];
fed_bank_2 setVariable ["safe",40,true];
fed_bank_3 setVariable ["safe",40,true];
fed_bank_4 setVariable ["safe",40,true];
...
"money_log" addPublicVariableEventHandler {
    _query = format ["INSERT INTO igLog (log) VALUES('%1')",(_this select 1)];

    [_query,1] call DB_fnc_asyncCall;
};
"advanced_log" addPublicVariableEventHandler {
    _query = format ["INSERT INTO igLog (log) VALUES('%1')",(_this select 1)];
...
life_airdrop = false;
publicVariable "life_airdrop";
life_airdropCount = 0;

//airdrop scheduler, checks if server should start an airdrop in intervals of 30-90 minutes
//aka earliest airdrop could start 30 minutes into restart, latest 90 minutes into restart
//the two airdrops could at the very least be 30 minutes apart, but this is unlikely
//TODO remove oec
[] spawn {
	_exit = false;
	while{true} do {
		if(!life_airdrop) then {//&& [] call OEC_fnc_timeUntilRestart >= 30
			uiSleep ((60*45)+(60*round(random 50)));
			if(count playableUnits > 18 && life_airdropCount < 2) then {
				[objNull] spawn TON_fnc_airdropServer;
				life_airdropCount = life_airdropCount + 1;
			} else {
				if(life_airdropCount > 1) exitWith {_exit = true;};
			};
			if(_exit) exitWith {};
		//} else {
