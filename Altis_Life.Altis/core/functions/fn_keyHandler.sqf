...
switch (_code) do {
    ...
    case 57: {
        ...
        if (_shift && {!(animationState player isEqualTo "AovrPercMrunSrasWrflDf")} && {isTouchingGround player} && {stance player isEqualTo "STAND"} && {speed player > 2} && {!life_is_arrested} && {((velocity player) select 2) < 2.5} && {time - jumpActionTime > 1.5}) then {
            ...
            if(currentWeapon player == "") then {
                [player] remoteExec ["life_fnc_jumpFnc2",RANY]; //Global execution
            } else {
    ...
    case 19: {
        ...
        if (_shift && life_inv_ziptie > 0 && playerSide isEqualTo civilian && {!isNull cursorObject} && {cursorObject isKindOf "CAManBase"} && {(isPlayer cursorObject)} && {(side cursorObject in [civilian,west])} && {alive cursorObject} && {cursorObject distance player < 3.5} && {!(cursorObject getVariable "Escorting")} && {!(cursorObject getVariable "restrained")} && {speed cursorObject < 1}) then {
            [] call life_fnc_restrainAction;
    ...
    case 24: {
        if (_shift) then {
            switch (player getVariable["Earplugs",0]) do {
                case 0: {
                    ["Vous avez mis vos boules quies à 90%",false,"fast"] call life_fnc_notification_system;
                    1 fadeSound 0.1;
                    player setVariable ["Earplugs", 10];
                };
                case 10: {
                    ["Vous avez mis vos boules quies à 60%",false,"fast"] call life_fnc_notification_system;
                    1 fadeSound 0.4;
                    player setVariable ["Earplugs", 40];
                };
                case 40: {
                    ["Vous avez mis vos boules quies à 30%",false,"fast"] call life_fnc_notification_system;
                    1 fadeSound 0.7;
                    player setVariable ["Earplugs", 70];
                };
                case 70: {
                    ["Vous avez enlevé vos boules quies",false,"fast"] call life_fnc_notification_system;
                    1 fadeSound 1;
