...
if ((lbCurSel ((findDisplay 3500) displayCtrl 3503)) isEqualTo -1)exitWith {["Vous devez d'abord choisir quelque chose.",true,"fast","orange","Error"]spawn Life_fnc_msg;};
...
if(_ctrl == "goldbar" && {!(Life_trunk_vehicle isKindOf "LandVehicle" OR Life_trunk_vehicle isKindOf "House_F")}) exitWith {["Vous ne pouvez stocker cet objet que dans des véhicules terrestres.",true,"fast","orange","Error"]spawn Life_fnc_msg;};

if(_ctrl == "money") then
{
    _index = [_ctrl,_inv] call TON_fnc_index;
    if(Life_cash < _num) exitWith {["Vous n'avez pas d'argent à stocker.",true,"fast","orange","Error"]spawn Life_fnc_msg;};
    if(_index == -1) then
    {
...
{
    if(((_totalWeight select 1) + _itemWeight) > (_totalWeight select 0)) exitWith {["Le véhicule est plein.",true,"fast","orange","Error"]spawn Life_fnc_msg;};

    if(!([false,_ctrl,_num] call Life_fnc_handleInv)) exitWith {["Impossible de déplacer l'élément.",true,"fast","orange","Error"]spawn Life_fnc_msg;};
    _index = [_ctrl,_inv] call TON_fnc_index;
