...
if ((("ToolKit" in (items player)) || playerSide in [west,independent]) && {alive life_vInact_curTarget} && {([life_vInact_curTarget] call life_fnc_isDamaged)}) then {_Btn1 ctrlEnable true;} else {_Btn1 ctrlEnable false;};
...
if (playerSide isEqualTo west) then {
    ...
    if (_curTarget isKindOf "Ship") then {
        ...
        if (!isNil "_id") then {
            ...
            _Btn2 buttonSetAction "life_vInact_curTarget setPos [getPos life_vInact_curTarget select 0, getPos life_vInact_curTarget select 1, (getPos life_vInact_curTarget select 2)+1.5]; closeDialog 0;";
