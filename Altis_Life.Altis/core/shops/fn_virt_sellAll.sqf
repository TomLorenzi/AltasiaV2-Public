...
if (LIFE_SETTINGS(getNumber,"player_advancedLog") isEqualTo 1) then {
    if (LIFE_SETTINGS(getNumber,"battlEye_friendlyLogging") isEqualTo 1) then {
        //advanced_log = format [localize "STR_DL_AL_boughtVehicle_BEF",_className,[_price] call life_fnc_numberText,[CASH] call life_fnc_numberText];
    } else {
        advanced_log = format [localize "STR_DL_AL_sellItem",profileName,(getPlayerUID player),[_amount] call life_fnc_numberText,_name,[_price] call life_fnc_numberText,[CASH] call life_fnc_numberText];
    };
