...
if ((player distance _area) > 10) exitWith { hint "Vous devez être plus pret du drapeau pour le capturer!"; }; //too far away
...
if (playerside isEqualTo west) exitWith {
	...
	//Police officer checks
	{
		...
		if (_areaContested && _xname isEqualTo "Rebels") then {
			hint "Rebelles proches!\nVous ne pouvez pas capturer!";
		};
	...
	if (!(_areaContested)) then {
		for "_i" from 0 to 1 step 0 do {
			sleep 2; //police capture time * 100
			_cP = _cP + _cpRate;
...
_xname = ""; //setup for popo name
_nearUnits = _area nearEntities ["Man",200]; //searches area for players
if (isNil {group player getVariable "gang_name"}) exitWith { hint "Vous devez être dans un gang pour capturer!"; }; //not in a gang
if (_group isEqualTo group player) exitWith { hint "Vous avez déjà le controle de cette zone!"; }; //already own it
if ((_area getVariable ["inCapture",FALSE])) exitWith {hint "Une seule personne à la fois!"; }; //stops 2 people capturing at the same time
...
//check if area is contested
{
    ...
    if (_areaContested) exitWith {
        if (_xname isEqualTo "Police Force") then {
            hint "Gendarmerie à proximité!\nVous ne pouvez pas capturer!";
        } else {
            hint "Rebelles à proximité!\nVous ne pouvez pas capturer!";
        };
    };
...
_gName = group player getVariable "gang_name";
_Text = format["Zone capturé par %1",_gName];
